...
def build_input_from_widgets() -> PrintJobInput:
    ...
    with st.sidebar.expander("默认报价参数", expanded=True):
        default_material_price = number_input("材料价格 / kg", 80.0, step=5.0)
        default_electricity_price = number_input("电价 / kWh", 0.8, step=0.1)
        default_machine_price = number_input("设备购置成本", 2500.0, step=100.0)
        default_machine_lifetime = number_input("设备折旧寿命（小时）", 2500.0, 1.0, step=100.0)
        default_maintenance = number_input("维护与耗材损耗 / 小时", 0.5, step=0.1)
        default_labor_rate = number_input("人工单价 / 小时", 35.0, step=5.0)
    ...
    with col_a:
        ...
        quantity = int(number_input("订单数量", 1, 1, step=1))
        material_price_per_kg = number_input("材料价格 / kg", default_material_price, step=5.0)
        model_weight_g = number_input("模型重量（g）", 85.0, step=5.0)
        material_waste_rate = number_input("支撑 / 损耗率（%）", 8.0, 0.0, 95.0, 1.0) / 100

    with col_b:
        st.subheader("打印与设备")
        print_hours = number_input("打印时间（小时）", 6.5, 0.01, step=0.5)
        printer_power_w = number_input("打印机平均功率（W）", 120.0, step=10.0)
        electricity_price_per_kwh = number_input("电价 / kWh", default_electricity_price, step=0.1)
        machine_purchase_price = number_input("设备购置成本", default_machine_price, step=100.0)
        machine_lifetime_hours = number_input("设备折旧寿命（小时）", default_machine_lifetime, 1.0, step=100.0)
        maintenance_cost_per_hour = number_input("维护与喷嘴等损耗 / 小时", default_maintenance, step=0.1)

    with col_c:
        st.subheader("人工与杂费")
        labor_hours = number_input("人工时间（小时）", 0.6, step=0.1)
        labor_rate_per_hour = number_input("人工单价 / 小时", default_labor_rate, step=5.0)
        packaging_cost = number_input("包装成本 / 单", 3.0, step=0.5)
        other_fixed_cost = number_input("其他固定成本 / 单", 2.0, step=0.5)

    with col_d:
        st.subheader("风险与报价")
        failure_rate = number_input("失败率（%）", 8.0, 0.0, 95.0, 1.0) / 100
        platform_fee_rate = number_input("平台佣金 / 手续费（%）", 6.0, 0.0, 80.0, 0.5) / 100
        target_profit_margin = number_input("目标利润率（%）", 30.0, 0.0, 90.0, 1.0) / 100
        override_unit_price = number_input(
            "手动售价 / 件（填 0 使用建议售价）",
            0.0,
            step=1.0,
